...
<head>
  ...
  <title>HỆ THỐNG ĐIỀU KHIỂN & GIÁM SÁT TỦ BƠM TƯỚI TIÊU</title>
  <!-- Thư viện Paho MQTT qua CDN -->
...
<body>
  ...
  <div class="container">
    ...
    <div class="card">
      <!-- Connection Panel -->
      <div id="connection-panel" style="margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border);">
        <div style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem; flex-wrap: wrap;">
            <select id="conn-method" onchange="toggleConnMethod()" style="background: rgba(0,0,0,0.2); border: 1px solid var(--border); color: white; padding: 0.5rem; border-radius: 0.5rem;">
                <option value="http">Mạng LAN (HTTP)</option>
                <option value="mqtt">Internet (MQTT)</option>
            </select>
            <!-- HTTP Input -->
            <div id="http-settings" style="display:flex; gap:0.5rem; flex:1;">
                <input type="text" id="device-ip-input" placeholder="IP ESP32 (vd: 192.168.1.10)" style="flex: 1; background: rgba(0,0,0,0.2); border: 1px solid var(--border); color: white; padding: 0.5rem; border-radius: 0.5rem;">
                <button onclick="saveIp()" class="btn-ctrl" style="background: var(--primary); color: white;">Lưu IP</button>
            </div>
            <!-- MQTT Input -->
            <div id="mqtt-settings" class="hidden" style="display:flex; gap:0.5rem; flex:1; flex-wrap:wrap;">
                <input type="text" id="mqtt-topic" placeholder="Topic (vd: tubom1)" style="flex: 1; min-width: 100px; background: rgba(0,0,0,0.2); border: 1px solid var(--border); color: white; padding: 0.5rem; border-radius: 0.5rem;">
                <button onclick="connectMQTT()" id="btn-mqtt-connect" class="btn-ctrl" style="background: var(--success); color: white;">Kết nối MQTT</button>
            </div>
        </div>
        <div id="conn-status" style="font-size: 0.75rem; color: var(--text-muted); margin-top: 5px;">Chế độ: LAN (HTTP)</div>
      </div>

      <h3><svg viewBox="0 0 24 24"><path d="M13 3h-2v10h2V3zm4.83 2.17l-1.42 1.42C17.99 7.86 19 9.81 19 12c0 3.87-3.13 7-7 7s-7-3.13-7-7c0-2.19 1.01-4.14 2.58-5.42L6.17 5.17C4.23 6.82 3 9.26 3 12c0 4.97 4.03 9 9 9s9-4.03 9-9c0-2.74-1.23-5.18-3.17-6.83z"/></svg> TRẠNG THÁI HỆ THỐNG</h3>
      
      <button id="btn-mode-toggle" class="mode-switch" onclick="toggleMode()">
        <span id="mode-icon">⚙️</span>
        <span id="mode-text">CHẾ ĐỘ: THỦ CÔNG</span>
      </button>
    ...
    <div id="manual-panel" class="card">
      ...
      <!-- SLAVE 2 -->
      <div class="pump-group-title">Khu Vực 2 (Slave 2)</div>
      ...
      <!-- MASTER -->
      <div class="pump-group-title">Bơm Tổng (Master)</div>
      <div class="pump-item" style="background: rgba(59, 130, 246, 0.05); border-color: rgba(59, 130, 246, 0.2);">
        <div class="pump-info">
          ...
          <span class="pump-name" style="color: var(--primary);">Bơm Chính</span>
        </div>
        <div class="btn-group">
    ...
    <div id="auto-panel" class="card hidden">
      <h3><svg viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/></svg> CẤU HÌNH TỰ ĐỘNG</h3>
      <p style="color: var(--text-muted); font-size:0.85rem; margin-bottom:1.5rem; line-height: 1.5;">
        Chu trình: Slave (20s) ➔ Master (30p)
      </p>
      
      <div class="config-row">
        <span class="config-label">Giờ chạy Sáng</span>
      ...
      <div class="config-row">
        <span class="config-label">Giờ chạy Chiều</span>
        <div class="time-input">
      ...
      <div class="config-row">
        <span class="config-label">Thời gian Bơm Master</span>
        <div class="time-input">
          <input type="number" id="mt" min="1" max="120">
          <span style="font-size: 0.8rem; color: var(--text-muted);">Phút</span>
        </div>
      </div>

      <button class="save-btn" onclick="saveConfig()">LƯU CÀI ĐẶT</button>
    </div>
  </div>

  <div id="toast">Đã lưu cài đặt!</div>

  <script>
    let currentMode = 0;
    let deviceIp = localStorage.getItem('esp32_ip');
    let mqttTopic = localStorage.getItem('mqtt_topic') || "tubom1";
    let useMqtt = false;
    let mqttClient = null;

    // Cấu hình MQTT Broker (Sử dụng Broker công cộng để test, thực tế nên dùng broker riêng)
    const MQTT_BROKER = "broker.hivemq.com";
    const MQTT_PORT = 8000; // Port WebSocket (WSS/WS)
    ...
    function toggleConnMethod() {
        ...
        if(method === 'mqtt') {
            document.getElementById('http-settings').classList.add('hidden');
            document.getElementById('mqtt-settings').classList.remove('hidden');
            document.getElementById('conn-status').innerText = "Chế độ: Internet (MQTT) - Cần kết nối";
            useMqtt = true;
        } else {
            document.getElementById('http-settings').classList.remove('hidden');
            document.getElementById('mqtt-settings').classList.add('hidden');
            document.getElementById('conn-status').innerText = "Chế độ: LAN (HTTP)";
            useMqtt = false;
        }
    }
    ...
    function connectMQTT() {
        ...
        if(!topic) return alert("Vui lòng nhập Topic!");
        ...
        mqttClient.onConnectionLost = (responseObject) => {
            document.getElementById('conn-status').innerText = "Mất kết nối MQTT: " + responseObject.errorMessage;
        ...
        mqttClient.onMessageArrived = (message) => {
            try {
                ...
                updateUI(data);
            } catch(e) { console.error("Lỗi parse JSON MQTT", e); }
        };

        document.getElementById('conn-status').innerText = "Đang kết nối MQTT...";
        
        mqttClient.connect({
            onSuccess: () => {
                document.getElementById('conn-status').innerText = "Đã kết nối MQTT Broker!";
                document.getElementById('conn-status').style.color = "var(--success)";
                mqttClient.subscribe(mqttTopic + "/status");
                showToast("Kết nối MQTT thành công!");
            },
            onFailure: (e) => {
                document.getElementById('conn-status').innerText = "Lỗi kết nối: " + e.errorMessage;
                document.getElementById('conn-status').style.color = "var(--danger)";
            },
            useSSL: false // HiveMQ public websocket thường không bắt buộc SSL, nếu dùng GitHub Pages (HTTPS) có thể cần true và port 8884
        });
    }

    function sendMqttCmd(subTopic, payload) {
        if (!mqttClient || !mqttClient.isConnected()) {
            showToast("Chưa kết nối MQTT!", "error");
            return;
        }
        const message = new Paho.MQTT.Message(payload.toString());
        message.destinationName = mqttTopic + "/" + subTopic;
    ...
    function updateUI(data) {
      ...
      document.getElementById('status-dot').className = "status-dot " + (data.status === "Đã kết nối" ? "active" : "");
      ...
      if(data.mode == 1) {
        document.getElementById('mode-text').innerText = "ĐANG CHẠY TỰ ĐỘNG";
        document.getElementById('mode-icon').innerText = "🤖";
        modeBtn.classList.add('auto');
        
        // Hiển thị panel điều khiển nhưng vô hiệu hóa thao tác (chỉ để xem trạng thái)
        const manualPanel = document.getElementById('manual-panel');
        manualPanel.classList.remove('hidden');
        manualPanel.style.pointerEvents = 'none'; // Không cho bấm
        manualPanel.style.opacity = '0.6'; // Làm mờ nhẹ
        document.querySelector('#manual-panel h3').lastChild.textContent = " GIÁM SÁT HOẠT ĐỘNG";

        document.getElementById('auto-panel').classList.remove('hidden');
      } else {
        document.getElementById('mode-text').innerText = "CHẾ ĐỘ: THỦ CÔNG";
        document.getElementById('mode-icon').innerText = "🖐️";
        modeBtn.classList.remove('auto');
        
        const manualPanel = document.getElementById('manual-panel');
        manualPanel.classList.remove('hidden');
        manualPanel.style.pointerEvents = 'auto'; // Cho phép bấm
        manualPanel.style.opacity = '1';
        document.querySelector('#manual-panel h3').lastChild.textContent = " BẢNG ĐIỀU KHIỂN";
    ...
    function fetchStatus() {
      if(useMqtt) {
          // MQTT nhận dữ liệu thụ động qua onMessageArrived, không cần polling chủ động
          // Tuy nhiên có thể gửi lệnh yêu cầu cập nhật nếu cần
      } else {
    ...
    function setMode(m) {
      if(useMqtt) sendMqttCmd("control", JSON.stringify({type: "mode", val: m}));
      else fetch(getApiUrl('/mode?val=' + m)).then(() => fetchStatus());
    }

    function toggleMode() {
      let newMode = currentMode == 1 ? 0 : 1;
      setMode(newMode);
      let targetMode = currentMode == 1 ? "THỦ CÔNG" : "TỰ ĐỘNG";
      if (confirm("Bạn có chắc chắn muốn chuyển sang chế độ " + targetMode + " không?")) {
        let newMode = currentMode == 1 ? 0 : 1;
        setMode(newMode);
      }
    }
    ...
    function ctrl(slave, pump, state) {
      if(useMqtt) sendMqttCmd("control", JSON.stringify({type: "pump", s: slave, p: pump, st: state}));
      else fetch(getApiUrl(`/control?slave=${slave}&pump=${pump}&state=${state}`));
    }
    
    function ctrlMaster(state) {
      if(useMqtt) sendMqttCmd("control", JSON.stringify({type: "master", st: state}));
      else fetch(getApiUrl(`/control_master?state=${state}`));
    }
    ...
    function saveConfig() {
      ...
      if(useMqtt) {
          sendMqttCmd("control", JSON.stringify({type: "config", mh, mm, eh, em, mt}));
          showToast("Đã gửi cài đặt qua MQTT!");
      } else {
          let url = getApiUrl(`/config?mh=${mh}&mm=${mm}&eh=${eh}&em=${em}&mt=${mt}`);
          fetch(url)
            .then(response => {
              if (response.ok) showToast("Đã lưu cài đặt thành công!", "success");
              else showToast("Lỗi khi lưu!", "error");
            })
            .catch(() => showToast("Mất kết nối!", "error"));
      }
    }
    ...
    function syncTime() {
      ...
      if(useMqtt) {
          sendMqttCmd("control", JSON.stringify({type: "time", y: d.getFullYear(), m: d.getMonth()+1, d: d.getDate(), h: d.getHours(), mi: d.getMinutes(), s: d.getSeconds()}));
          showToast("Đã gửi lệnh đồng bộ thời gian!");
      } else {
          const url = getApiUrl(`/sync_time?y=${d.getFullYear()}&m=${d.getMonth()+1}&d=${d.getDate()}&h=${d.getHours()}&mi=${d.getMinutes()}&s=${d.getSeconds()}`);
          fetch(url)
            .then(r => {
              if(r.ok) showToast("Đã đồng bộ thời gian!", "success");
              else showToast("Lỗi đồng bộ!", "error");
            })
